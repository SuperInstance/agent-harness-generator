...
async function loadWasm(): Promise<KernelBackend> {
  // @ts-ignore — generated by `wasm-pack build` into ../pkg/, may be
  // absent during a TS-only build (the WASM CI job builds it separately).
  // The runtime loader gracefully falls back to the NAPI backend if the
  // dynamic import fails, so a build-time path miss is non-fatal.
