...
| [ADR-032](./ADR-032-host-copilot.md) | GitHub Copilot as a harness host | Implemented (iter 127) | `@ruflo/host-copilot` — the 7th host adapter. Emits `.vscode/mcp.json` (both `servers` + `mcpServers` keys for VSCode 1.99+ AND Claude Code compat). MCP policy layer unchanged. |
| [ADR-033](./ADR-033-host-github-actions.md) | GitHub Actions as a harness host | Implemented (iter 146–147) | `@ruflo/host-github-actions` — headless CI/CD host, the first NON-INTERACTIVE one. Composite action in `.github/actions/<name>/`, workflow YAML in `.github/workflows/<name>.yml`. Default-deny via the workflow `permissions:` block (`permissionsBlock()` maps allow-list → least-privilege GITHUB_TOKEN scopes). 9th host; propagated into HOSTS catalog + bench + web-ui + CI; `metaharness@0.1.5`. |
